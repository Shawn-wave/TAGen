    ...
    def generate_images(self, input_file_path, prompts, num_images=20):
        ...
        for category in ['background', 'table']:
            ...
            for i in tqdm(range(num_images), desc=f"Generating {category} images"):
                augmented_image, success = self.genaug.aug_image(frame_rgb, mask, prompt, category)
                if success and augmented_image.shape[:2] != mask.shape:
                ...
                full_image[..., 3] = mask
                full_image[mask == 255, :3] = augmented_image[mask == 255]
...
class ImageCompositor:
    ...
    def composite_images(self, input_file_path, iteration):
        ...
        with h5py.File(input_file_path, 'r') as input_hf, h5py.File(h5_output_path, 'w') as output_hf:
            ...
            background_image = np.array(Image.open(os.path.join(self.generated_dir, 'background', background_image_path)))
            table_image = np.array(Image.open(os.path.join(self.generated_dir, 'table', table_image_path)))
