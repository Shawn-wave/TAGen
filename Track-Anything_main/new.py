...
def vos_tracking_video(video_state, interactive_state, mask_dropdown, save_dir):
    ...
    for mask_name in mask_dropdown:
        empty_frame_index = find_first_empty_mask_frame(masks_dict[mask_name])
        if empty_frame_index is not None:
            empty_frame_indices[mask_name] = empty_frame_index
        else:
            empty_frame_indices[mask_name] = None

    earliest_empty_frame = None
    for mask_name, empty_frame in empty_frame_indices.items():
        if empty_frame is not None:
            if earliest_empty_frame is None or empty_frame < earliest_empty_frame:
                earliest_empty_frame = empty_frame

    if earliest_empty_frame is not None:
        last_valid_frame_index = earliest_empty_frame - video_state["select_frame_number"] - 1
    else:
    ...
    video_output = generate_video_from_frames(video_state["painted_images"], output_path=f"./result/track/{video_state['video_name']}", fps=fps)
    interactive_state["inference_times"] += 1
    
    print(f"For generating this tracking result, inference times: {interactive_state['inference_times']}, "
          f"click times: {interactive_state['positive_click_times'] + interactive_state['negative_click_times']}, "
          f"positive: {interactive_state['positive_click_times']}, "
          f"negative: {interactive_state['negative_click_times']}")
    
    operation_log = []
    for mask_name, empty_frame in empty_frame_indices.items():
        if empty_frame is not None:
            message = f"Empty frame detected for {mask_name} at frame {empty_frame}. Please add new mask."
            print(message)
            operation_log.append((message, "Warning"))
        else:
            message = f"No empty frame detected for {mask_name}."
            print(message)
            operation_log.append((message, "Info"))

    return video_output, video_state, interactive_state, operation_log, gr.update(value=earliest_empty_frame, visible=earliest_empty_frame is not None)
...
def save_frame(video_state, masks_dict, interactive_state, output_path):
    with h5py.File(output_path, 'a') as hf:
        ...
        for mask_name, masks in masks_dict.items():
            ...
            # 새로운 마스크 시작 전의 프레임들을 0으로 채움
            stacked_masks.extend([np.zeros_like(masks[0]) for _ in range(start_frame)])
            ...
            if stacked_masks:
                ...
                hf[mask_dataset_name].attrs['EndFrame'] = total_frames - 1  
            
        hf.attrs['last_processed_frame'] = total_frames - 1

    print(f"Masks saved to {output_path}")
    print(f"Total frames: {total_frames}")
    for mask_name, masks in masks_dict.items():
        print(f"Frames for {mask_name}: {len(masks)}, Starting from frame {start_frame}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=int, default=5900, help="The port number for the server")
