...
def sam_refine(video_state, point_prompt, click_state, interactive_state, evt:gr.SelectData):
    ...
    # <Change> save to binary
    frame_number = video_state["select_frame_number"]
    h5_filename = f"non_clicked_mask_frame_{frame_number}.h5"
    
    with h5py.File(h5_filename, 'w') as hf:
        hf.create_dataset("mask", data=non_clicked_mask, dtype='bool', compression="gzip")
...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    ...
    # <Change> Generate non-clicked masks for all frames
    non_clicked_masks = [np.logical_not(mask) for mask in masks]  
    
    # Save all non-clicked masks to a single H5 file
    h5_filename = f"non_clicked_masks_{video_state['video_name']}.h5"
    with h5py.File(h5_filename, 'w') as hf:
        for frame_idx, non_clicked_mask in enumerate(non_clicked_masks):
            frame_number = video_state["select_frame_number"] + frame_idx
            hf.create_dataset(f"frame_{frame_number}", data=non_clicked_mask, dtype='bool', compression="gzip")
    
    print(f"All non-clicked masks saved to {h5_filename}")
